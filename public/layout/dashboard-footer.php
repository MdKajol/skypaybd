<!-- footer start -->
<footer class="dashboard-footer menu-left-gap">
	<div class="footer-area pb-20 pt-20">
		<div class="container">
			<div class="row align-items-center">
				<div class="col-xl-6 col-lg-6 col-md-6">
					<div class="copyright">
						<p>OlympiaBet.it &copy; 2018 All Rights Are registred</p>
					</div>
				</div>
				<div class="col-xl-6 col-lg-6 col-md-6">
					<div class="footer-social text-lext text-md-right">
						<a href="https://facebook.com/page">
							<i class="icofont icofont-social-facebook"></i>
						</a>
						<a href="https://skype.com/">
							<i class="icofont icofont-social-skype"></i>
						</a>
						<a href="https://twitter.com/">
							<i class="icofont icofont-social-twitter"></i>
						</a>
					</div>
				</div>
			</div>
		</div>
	</div>
</footer>
<!-- footer end -->

<script src="<?php echo link_script("vendor/jquery-1.12.4.min.js"); ?>"></script>
<script src="<?php echo link_script("popper.min.js"); ?>"></script>
<script src="<?php echo link_script("bootstrap.min.js"); ?>"></script>
<script src="<?php echo link_script("slick.min.js"); ?>"></script>
<script src="<?php echo link_script("input-validate.js"); ?>"></script>
<script src="<?php echo link_script("ku-main.js"); ?>"></script>
<script src='https://www.google.com/recaptcha/api.js'></script>
</body>
</html>
